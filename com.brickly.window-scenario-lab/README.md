...
8. **命令短命**：`open-*` 命令立刻 return；窗靠 `runtime.ui` + `keepAlive` 的 Session 绑定保活。控制台里再开场景走 `brick.invoke('open-scenario')` 演示命令路径（expose 回调里也可直接用 `runtime.ui` 开 Session 窗）。
...
- 命令结束即释放这次 invoke；独立窗靠 Session 绑定 + `keepAlive` 留下。控制台开新窗走 `brick.invoke` 演示命令路径。
