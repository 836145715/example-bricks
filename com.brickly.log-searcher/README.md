...
UI 结果区默认使用自动换行虚拟列表，通过动态行高测量展示长日志；右下角提供“到顶部 / 到底部”快捷按钮，便于在大结果集内显式跳转。需要高密度横向浏览时，可切换为“单行”模式。Ctrl+F 会高亮当前已加载视图窗口，并可通过上一个/下一个在当前 Tab 的 Go 侧已存结果中定位跳转；复制按钮只复制当前已加载视图窗口，不会一次性拉取全部结果。
