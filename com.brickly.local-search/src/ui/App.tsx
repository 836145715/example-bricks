  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        // StrictMode 会双跑 effect：本次已被 cleanup 取消时，只交回自己这一柄。
        // 宿主侧 handleId 与 session 解耦，交回旧柄不会影响后来者的占用。
        if (cancelled) {
    ...
    return () => {
      ...
      bindRuntime(null)
      // 只交回自己这一柄。宿主侧 handleId 与 Session 解耦：
      // 若该 Session 已被后来者认领，releaseClaim 会因 generation 过期而 no-op。
