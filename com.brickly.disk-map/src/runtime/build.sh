...
RUNTIME_ROOT="$ROOT/out/runtime"
STAMP="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
TARGETS="${1:-mac-arm64,mac-x64}"

build_one() {
  local key="$1" goos="$2" goarch="$3" suffix="$4"
  local out_dir="${BRICKLY_BUILD_OUT:-$RUNTIME_ROOT/$key}"
  mkdir -p "$out_dir"
...
for key in "${items[@]}"; do
  ...
  case "$key" in
    ...
    *) echo "Unknown target: $key (disk-map is macOS only)" >&2; exit 1 ;;
  esac
done
echo Done.
