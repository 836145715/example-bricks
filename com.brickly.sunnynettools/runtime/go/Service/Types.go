		...
		for {
			...
			for len(InsertList) > 0 {
				...
				for i := range batch {
					...
					captureSetFlags(batch[i].Theology, batch[i].BreakMode, batch[i].GuaranteeDisplay)
				}
				captureAppendBatch(ids)
				// 把原版 Insert 批次推给 capture-stream，前端走 insertArray。
				// 不要 Events.Publish("insert")：那会变成宿主「通知事件」，主列表已不再订阅。
				captureBroadcast(map[string]any{"type": "insert", "rows": batch})
			}
