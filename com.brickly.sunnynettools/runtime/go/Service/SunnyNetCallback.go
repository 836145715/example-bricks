...
func (g *AppMain) AppImport(filePath string) (err string, list []Session.Insert) {
	...
	return "", Session.Import(brReader, func(obj Session.AppSession) Session.Insert {
		ins := InsertFuck(obj)
		if ins.Theology != 0 {
			captureAppend(ins.Theology)
			captureSetFlags(ins.Theology, ins.BreakMode, ins.GuaranteeDisplay)
		}
		return ins
	}, g.progress)
}
