        ...
        //响应 图片视图
        {
          ...
          if (this.ResponseTabs[3].Show) {
            if (this.Stream.Response.IsMaxLength) {
              GetHTTPResponseBody(parseInt(this.SelectedRow.Theology), true).then((data) => asBodyBytes(data)).then((bytes) => {
                this.$refs.ResponseImg.SetImg(bytesToBase64(bytes), ImageType)
              })
            } else {
