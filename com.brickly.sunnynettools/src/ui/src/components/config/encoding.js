...
export function isBricklyResourceRef(value) {
    return Boolean(
        value &&
        typeof value === "object" &&
        !Array.isArray(value) &&
...
/** 把 GetHTTP*Body 的返回值变成 Uint8Array：截断预览是 base64 字符串，超限完整包是 ResourceRef。 */
export async function asBodyBytes(payload) {
    if (payload == null) {
        return new Uint8Array(0);
    }
    ...
    if (isBricklyResourceRef(payload)) {
        const resources = window.brickly?.resources;
        if (!resources || typeof resources.open !== "function") {
            throw new Error("window.brickly.resources 不可用");
        }
        const handle = resources.open(payload);
        try {
