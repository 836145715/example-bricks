...
function clipCopy(str) {
  ClipboardWriteAll(str).then((res) => {
    if (res === '') {
      ElMessage({
        message: '已成功复制到剪贴板',
        type: 'success',
...
let BeyondLength = 5000;
export default {
  props: ["backcolor", "Name"],
  watch: {
    backcolor(backcolor) {
  ...
  data() {
    return {
      ...
      color: {
        ...
        Header: "#e8e8e8",
        Body: "#030101",
        prompt: "color: rgba(2,2,5,0.93);position: relative;top:3px;color:#000000;justify-content: center;display:flex;",
        Select: {
  ...
  computed: {
    Theme() {
      // 获取 .monaco-editor 元素的所有计算后的CSS属性
      try {
        const monacoEditorStyle = window.getComputedStyle(document.querySelector('.monaco-editor'));
        // 获取特定的CSS属性值
        const editorBackground = monacoEditorStyle.getPropertyValue('--vscode-editor-background');
        const editorGutterBackground = monacoEditorStyle.getPropertyValue('--vscode-editorGutter-background');
        const editorSelectionBackground = monacoEditorStyle.getPropertyValue('--ag-range-selection-border-color');
        const editorSelectionRowBackground = monacoEditorStyle.getPropertyValue('--ag-selected-row-background-color');
      ...
      if (this.theme) {
        ...
        this.color.prompt = "color: rgba(2,2,5,0.93);position: relative;top:" + (this.Window.Height - 23) + "px;color:#ffffff;justify-content: center;display:flex;"
        ...
        this.color.prompt = "color: rgba(2,2,5,0.93);position: relative;top:" + (this.Window.Height - 23) + "px;color:#000000;justify-content: center;display:flex;"
    ...
    IsWindows() {
      if (window.Theme) {
        return Config_GOOS_IsWindows.value ? "width: 182px;" : "width: 200px;"
      }
      return "width: 182px;"
    }
  },
  mounted() {
    ...
    const elementRef = this.$refs.HexThis;
  ...
  methods: {
    ElMessageError(message) {
      this.$refs.myIndex.focus()
      this.$refs.myIndex.value = "";
      const currentTime = Date.now();
      if (currentTime - this.ErrorTime < 100) {
        return
      }
    ...
    GetALLBytes() {
      ...
      return bytesToRequest(mergedBytes);
    },
    GetResponseBytes() {
      if (!this.IsHasModify) {
        return null;
      ...
      return bytesToResponse(mergedBytes);
    },
    async copy(option, radio) {
      if (option === "select") {
        return null;
      }
      let obj = [];
      if (this.Name === "RequestHex") {
        obj = this.GetALLBytes();
      } else if (this.Name === "ResponseHEX") {
        obj = this.GetResponseBytes()
      } else {
        return null;
      }
    ...
    async menuItemClicked(option) {
      ...
      if (option === "all") {
        ...
        if (this.radio === "十六进制") {
          ...
          clipCopy(bytesToBase64(mergedBytes))
        }
        ...
        if (this.radio === "十六进制") {
          ...
          clipCopy(bytesToBase64(SelectBody))
        }
      } else if (option === "body") {
        let mergedBytes = []
        if (this.RawBeyond) {
    ...
    adjustWidth() {
      ...
      let charWidth = Config_GOOS_IsWindows.value ? 6 : 7.5
    ...
    handleHexPaste(event) {
      if (event.command === 10) {
        ClipboardReadAll().then(pastedText => {
          if (isHexadecimalString(pastedText)) {
            ...
            requestAnimationFrame(() => {
              this.ElMessageError('粘贴的不是 十六进制字符串');
            })
          }
        })
            .catch(error => {
              requestAnimationFrame(() => {
                this.ElMessageError('获取剪贴板内容出错:' + error);
              })
            });
...
<template>
  <div ref="HexThis" style="width: 100%;height: 100%">
    <div ref="myHexDiv" :style="CSS_HexDiv">
      <div ref="myIndex" :style="CSS_Index" class="Hex_Index_style" contenteditable="false"
    ...
    <span :style="color.prompt" v-if="this.RawBeyond">{{ this.BeyondText }}</span>
    <div v-show="isContextMenuVisible" ref="menu">
      <el-menu
          ...
          text-color="#fff"
      >
        <el-radio-group v-model="radio" size="small" class="parent-div">
          <el-radio-button value="十六进制" style="width: auto">十六进制</el-radio-button>
          <el-radio-button value="Base64" style="width: auto">Base64</el-radio-button>
          <el-radio-button value="Ansi" style="width: auto">Ansi</el-radio-button>
        </el-radio-group>

        <el-menu-item @click="menuItemClicked( 'all')" index="1">复制全部</el-menu-item>
        <el-menu-item @click="menuItemClicked( 'select') " index="2">复制选中</el-menu-item>
        <el-menu-item @click="menuItemClicked( 'body')"  index="3">复制全部Body</el-menu-item>
