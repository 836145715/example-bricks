...
// PersistSave / PersistClear 由 main 在 OnReady 注入：双写到宿主 Storage（user scope）。
var (
	PersistSave  func(doc []byte)
	PersistClear func()
)
...
func (f *config) Save() {
	...
	_ = os.WriteFile(filepath.Join(homeDir, "SunnyNet.json"), _config, 0600)
	if PersistSave != nil {
		PersistSave(_config)
	}
}
func (f *config) Reset() {
	...
	_ = os.WriteFile(filepath.Join(homeDir, "SunnyNet.json"), []byte("{}"), 0600)
	Config = config{
		ListColor:    make(map[string]string),
		RequestCert:  make(map[int]*CertInfo),
		ReplaceRoles: make(map[int]*ReplaceBodyInfo),
