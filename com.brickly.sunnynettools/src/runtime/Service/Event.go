...
func (g *AppMain) ClipboardWriteAll(value string) string {
	if PlatformClipboardWrite != nil {
		if err := PlatformClipboardWrite(value); err == nil {
			return ""
		}
	}
...
func (g *AppMain) GetHTTPResponseBody(Theology int, getAllBody bool) any {
	defer func() {
		if err := recover(); err != nil {
			panic(err)
		}
	}()
	return httpBodyPayload(Theology, false, getAllBody)
}
func (g *AppMain) GetHTTPRequestBody(Theology int, getAllBody bool) any {
	defer func() {
		if err := recover(); err != nil {
			panic(err)
		}
	}()
	return httpBodyPayload(Theology, true, getAllBody)
}
...
func (g *AppMain) AppDeleteSession(tid []int) {
	...
	lock.Unlock()
	captureBroadcastTheologies("delete", remove)
	return
}
...
func (g *AppMain) ClearAllSession() []int {
	...
	lock.Unlock()
	// 数据面 Phase 1：索引清空 + 广播 clear（连接中的会话后续重新插入走 insert 增量）
	captureClearIndex()
	captureBroadcast(map[string]any{"type": "clear"})
	return array
}
