...
var bridgeOpCatalog = []BridgeOpCapability{
	...
	{Op: "session_pack_export", Description: "【场景】导出单条 WebSocket/TCP 会话的流消息包（.bin），与 ExportMessage 同源。", Args: "filePath（.bin 绝对路径） theology | rowId | listIndex", Returns: "{\"ok\":true,\"path\":string,\"theology\":n,\"rowId\":string}；HTTP 会话不支持。"},
