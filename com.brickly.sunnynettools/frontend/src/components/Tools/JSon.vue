...
export default {
  ...
  methods: {
    SetCode(newValue) {
      ...
      try {
        let _Body = toUTF8(newValue)
        if (_Body.indexOf("�") !== -1) {
          _Body = toGBK(newValue)
        }
        const c = _Body
    ...
    GetCode() {
      if (this.AddElement.Box.checked) {
        let _Body = toUTF8(this.Raw)
        if (_Body.indexOf("�") !== -1) {
          _Body = toGBK(this.Raw)
        }
        return _Body
      }
    ...
    GetRawCode() {
      ...
      return bytesToBase64(StringToBytes(JSON.stringify(this.jsonData)));
    },
    IsHasModify() {
      try {
        let _Body = toUTF8(this.Raw)
    ...
    pbConvert(pb, skip) {
      this.ReadOnly = true
      ProtobufToJson(bytesToBase64(pb), parseInt(skip)).then(response => {
        try {
          if (response === 'null' || response === '') {
            this.jsonData = {"Data": "无效的 Protobuf 数据"}
            this.$nextTick(() => {
              this.eventClick()
            })
            return
          }
          if (response.length > 10240 * 300) {
          ...
          this.jsonData = {"Data": " Protobuf 数据", "Error": e}
          this.$nextTick(() => {
            this.eventClick()
          })
        }
...
<style>
.jse-theme-dark {
  --jse-panel-background: var(--ag-background-color) !important;
  --jse-background-color: var(--ag-background-color) !important;
}
