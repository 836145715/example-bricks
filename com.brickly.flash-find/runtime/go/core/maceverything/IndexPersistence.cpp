    ...
    static constexpr size_t kWALHeaderSize = 2 * sizeof(uint32_t); // magic + version
    {
        ...
        if (force) {
            if (wal_->currentSize() <= kWALHeaderSize) {
                // Fresh-scan case: WAL is empty but no base index exists yet.
                // The owned process is short-lived and cannot rely on the 300s
                // auto-compaction timer, so a forced flush must write the base.
                namespace fs = std::filesystem;
                if (fs::exists(v6Path_) || fs::exists(basePath_)) {
                    LOG_INFO("IndexPersistence", "Skipping flush — WAL is empty");
                    return;
                }
                LOG_INFO("IndexPersistence", "WAL is empty but no base index exists — writing full snapshot");
            }
        } else {
            if (!wal_->isDirty()) {
