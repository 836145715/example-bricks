...
Chrome 会继承 macOS 系统代理，但不会因为 Inspector 正在监听就自动使用它。设置代理后，用 Chrome 访问 `https://example.com` 验证；`localhost`、`127.0.0.1` 等地址通常会被浏览器绕过，不适合作为验证目标。macOS 的证书文件位于 `~/.mitmproxy/mitmproxy-ca-cert.cer`，安装后可在“钥匙串访问”中将它设置为“始终信任”。
