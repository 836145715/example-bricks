...
`com.brickly.lan-share` 是一个把本机目录通过 HTTP 共享到局域网的工具 Brick。体验窗占用 `owned` runtime，webview 负责配置、显式启停 HTTP 文件服务与状态展示。关窗即释放进程，不作为后台服务驻留。

## 生命周期

- 打开工具会 `window.brickly.start()` 占用 owned runtime，只读取 HTTP 共享状态，不会自动开始共享。
- 点击「启动共享」后，UI 在已占用的 runtime 内启动 HTTP 文件服务。
- 点击「停止共享」只停止 HTTP 文件服务；体验窗仍占用 runtime，可再次启动。
- 关闭工具窗口会 `dispose()` owned runtime，HTTP 文件服务随之停止。
- Brickly 主程序退出后不会保留共享；下次启动也不会自动恢复。

共享目录、端口、上传开关和“是否已设置访问码”会缓存在 webview 的 `localStorage`，访问码明文不会写入该缓存；访问码输入留空时保留 runtime 中的原值。
